...
document.addEventListener("input", event => {
  if (event.target.matches(".remaining")) {
    PRODUCTS.find(product => product.id === event.target.dataset.id).remaining = num(event.target.value);
...
document.addEventListener("change", event => {
  ...
  if (event.target.matches("[data-field]")) {
    loadConfig[num(event.target.dataset.index)][event.target.dataset.field] = event.target.value;
    renderSlots(); renderAfterDelivery();
  }
});
