...
function renderProducts() {
  ...
  PRODUCTS.forEach(product => {
    ...
    const loadFill = Math.min((load / product.capacity) * 100, 100 - fill);
    const tankFill = node.querySelector(".tank-fill");
    tankFill.style.setProperty("--fill", fill + "%");
    tankFill.style.background = product.color;
    // layer สีเหลือง pastel แสดง order ที่เพิ่มเข้ามา วางซ้อนบนสีเดิม
    const tankOrder = node.querySelector(".tank-order");
    if (tankOrder) {
      tankOrder.style.setProperty("--order-fill", loadFill + "%");
      tankOrder.style.setProperty("--order-bottom", fill + "%");
      tankOrder.style.display = load > 0 ? "block" : "none";
    }
    node.querySelector(".tank-percent").textContent = `ความจุถัง ${fmt(product.capacity)} ลิตร`;
    ...
    area.append(node);
    // อัปเดต tank-order layer หลัง append (node ถูก consume แล้ว ใช้ lastElementChild)
    const card = area.lastElementChild;
    const load = productLoad(product.id);
    const loadFill = Math.min((load / product.capacity) * 100, 100 - fill);
    const tankOrder = card.querySelector(".tank-order");
    if (tankOrder) {
      tankOrder.style.setProperty("--order-fill", loadFill + "%");
      tankOrder.style.setProperty("--order-bottom", fill + "%");
      tankOrder.style.display = load > 0 ? "block" : "none";
...
function renderAfterDelivery() {
  ...
  // sync tank-order layer ทุก card เมื่อ load config เปลี่ยน
  document.querySelectorAll(".product").forEach(card => {
    const inp = card.querySelector(".remaining");
    if (!inp) return;
    const product = PRODUCTS.find(p => p.id === inp.dataset.id);
    if (!product) return;
    const fill = Math.min((product.remaining / product.capacity) * 100, 100);
    const load = productLoad(product.id);
    const loadFill = Math.min((load / product.capacity) * 100, 100 - fill);
...
document.addEventListener("input", event => {
  if (event.target.matches(".remaining")) {
    ...
    if (card) {
      ...
      const load = productLoad(product.id);
      const loadFill = Math.min((load / product.capacity) * 100, 100 - fill);
      card.querySelector(".tank-fill").style.setProperty("--fill", fill + "%");
      const tankOrder = card.querySelector(".tank-order");
      if (tankOrder) {
        tankOrder.style.setProperty("--order-fill", loadFill + "%");
        tankOrder.style.setProperty("--order-bottom", fill + "%");
